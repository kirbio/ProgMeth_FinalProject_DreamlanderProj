...
public class DrawingUtility {
	...
	public static final int ATK_GAUGE_Y = STATUS_BAR_HEIGHT; // Adjacent to
																// Status bar
	...
	/*
	 * ================================================= Drawing GUI - TextArea,
	 * StatusBar, Attack Gauge, counter
	 * ==================================================
	 */
	...
	public static void drawStatusBar(GraphicsContext gc, int level, int HP, int maxHP, int plLevel, int exp,
			int nextExp) {
		// draw Status bar
		gc.setFill(Color.BLACK);
	...
	public static void drawAttackGauge(GraphicsContext gc, int[] attackpower, int index, int currentAtkPower,
		...
		if (showAttackDescription) {
			...
			gc.fillText(AttackGuage.attackDescription[currentAtkPower], ATK_GAUGE_TEXT_X,
					ATK_GAUGE_BASE_Y - (cellHeight * (index + 1)) + cellHeight / 2);

			// gc.setStroke(Color.BLACK);
			// gc.setLineWidth(1);
			// gc.strokeText(AttackGuage.attackDescription[currentAtkPower],
			// ATK_GAUGE_TEXT_X, ATK_GAUGE_BASE_Y - (cellHeight*(index+1)));
	...
	public static void drawCounter(GraphicsContext gc, int startTime, int currentTime) {
		...
		for (int i = 0; i < currentTime; i++) {
			gc.fillRoundRect(10, (ATK_GAUGE_Y + ATK_GAUGE_HEIGHT - 10) - (cellHeight * (i + 1)), COUNTER_WIDTH - 20,
					cellHeight, 10, 10);
			gc.strokeRoundRect(10, (ATK_GAUGE_Y + ATK_GAUGE_HEIGHT - 10) - (cellHeight * (i + 1)), COUNTER_WIDTH - 20,
					cellHeight, 10, 10);
			// gc.strokeRect(ATK_GAUGE_CELL_X, ATK_GAUGE_BASE_Y -
			// (cellHeight*(i+1)), ATK_GAUGE_CELL_W, cellHeight);
		}
